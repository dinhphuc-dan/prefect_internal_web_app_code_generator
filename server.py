...
from prefect_airbyte_connections_orchestration import (
    GeneratePrefectAirbyteJinjaTemplate,
)
...
def git_check(clicked_button_status=None):
    ...
    prefect_airbyte_clicked_button_status = request.form.get(
        "prefect_airbyte_check_button", None
    )
    prefect_dbt_clicked_button_status = request.form.get(
        "prefect_dbt_check_button", None
    )
    ...
    if request.method == "POST":
        if prefect_airbyte_clicked_button_status == "true":
            github_name = (
                re.search(
                    r"\/[\S]+", os.getenv("GIT_PREFECT_AIRBYTE_ORCHESTRATION_NAME")
                )
                .group()
                .split("/")[1]
            )
            ...
            github_name = (
                re.search(
                    r"\/[\S]+", os.getenv("GIT_PREFECT_DBT_CORE_ORCHESTRATION_NAME")
                )
                .group()
                .split("/")[1]
            )
            ...
            github_name = (
                re.search(r"\/[\S]+", os.getenv("GIT_SUPERSET_AUTOMATION_NAME"))
                .group()
                .split("/")[1]
            )
            path_to_git = Path.cwd() / github_name
        ...
        try:
            ...
            run = subprocess.run(
                command, shell=True, cwd=path_to_git, capture_output=True, check=True
            )
...
def generate_prefect_dbt_core_template():
    ...
    if request.method == "POST" and clicked_button_status == "true":
        try:
            ...
            dbt_core_object_name_in_prefect: str = request.form[
            ...
            log = text_formatter(
                command=command,
                text=f"generate file {dbt_object.file_name} Succeessfully",
            )
            ...
            session["prefect_dbt_generated_file_location"] = str(
                dbt_object._write_file_location
            )
            ...
            action_status = "Failed"
        return render_template(
            "generate_prefect_dbt_core_template.html",
            log=log,
            action_name=action_name,
            action_status=action_status,
        )
        ...
        try:
            if request.form["dbt_command"].strip():
                ...
                run = subprocess.run(
                    command,
                    shell=True,
                    cwd=Path(session.get("prefect_dbt_generated_file_location")),
                    capture_output=True,
                    check=True,
                )
                log = text_formatter(command=command, text=repr(run))
            ...
            action_status = "Failed"
        return render_template(
            "deploy_dbt_command_to_prefect.html",
            log=log,
            action_name=action_name,
            action_status=action_status,
        )
...
def generate_prefect_airbyte_template():
    ...
    if request.method == "POST" and clicked_button_status == "true":
        try:
            ...
            airbyte_object_name_in_prefect: str = request.form[
                "airbyte_object_name_in_prefect"
            ].strip()
            ...
            log = text_formatter(
                command=command,
                text=f"generate file {airbyte_object.file_name} Succeessfully",
            )
            ...
            session["prefect_airbyte_generated_file_location"] = str(
                airbyte_object._write_file_location
            )
            ...
            action_status = "Failed"
        return render_template(
            "generate_prefect_airbyte_template.html",
            log=log,
            action_name=action_name,
            action_status=action_status,
        )
    ...
    if request.method == "POST" and clicked_button_status == "true":
        try:
            ...
            run = subprocess.run(
                command,
                shell=True,
                cwd=Path(session.get("prefect_airbyte_generated_file_location")),
                capture_output=True,
                check=True,
            )
            log = text_formatter(command=command, text=repr(run))
            ...
            action_status = "Failed"
        return render_template(
            "deploy_airbyte_flow_to_prefect.html",
            log=log,
            action_name=action_name,
            action_status=action_status,
        )
...
def superset_automation_options():
    list_companies = ["govo", "jacat", "fidra", "pion"]
    # when user click button, flask will send a post request
    create_new_roles_and_role_security_button_status = request.form.get(
        "create_new_roles_and_role_security", None
    )
    update_table_all_role_sercurity_button_status = request.form.get(
        "update_table_all_role_sercurity", None
    )
    update_mkt_app_permissions_button_status = request.form.get(
        "update_mkt_app_permissions", None
    )

    if request.method == "POST":
        if create_new_roles_and_role_security_button_status == "true":
            list_role_type_options = [
                "app_related_role",
                "mkt_performance_related_role",
            ]
            ...
            action_id = 3
        return render_template(
            "superset_automation_options.html",
...
def superset_automation_actions_loading():
    session["superset_role_type_option"] = request.form.get(
        "superset_role_type_options", None
    )
...
def superset_automation_actions_result():
    ...
    (
        ...
        company_name,
    ) = handle_user_input(fuction_type=role_type_option, company_name=company)
    bg = Bigquery()
    ...
    try:
        ...
        if action_id == "1":
            ...
            with redirect_stdout(f):
                superset.update_table_all_row_level_security(
                    tables_schema=row_level_security_id_query_condition,
                    list_dataset=list_dataset,
                    table_id_query_condition=table_id_query_condition,
                )
        elif action_id == "3":
            with redirect_stdout(f):
                superset.update_users_app_permission(
                    objects=list_object,
                    company_name=company_name,
                    bigquery_connection=bg,
                )
        ...
        action_status = "Failed"
    return render_template(
        "superset_automation_actions_result.html",
        log=log,
        action_name=action_name,
        action_status=action_status,
    )
